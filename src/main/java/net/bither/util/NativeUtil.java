...
public class NativeUtil {
    ...
    public static void compressBitmap(Bitmap bit, int quality, String fileName,
        ...
        if (bit.getWidth() > 3000) {
            quality = 200;
            result = Bitmap.createBitmap(bit.getWidth() / 6, bit.getHeight() / 6,
                    Config.ARGB_8888);//
//		result= UtilsClass.comp(result);
            Canvas canvas = new Canvas(result);
            Rect rect = new Rect(0, 0, bit.getWidth(), bit.getHeight());// original
            rect = new Rect(0, 0, bit.getWidth() / 6, bit.getHeight() / 6);//
            canvas.drawBitmap(bit, null, rect, null);

        } else if (3000 > bit.getWidth() && bit.getWidth() > 1400) {
            quality = 100;
            result = Bitmap.createBitmap(bit.getWidth() / 5, bit.getHeight() / 5,
                    Config.ARGB_8888);// 缩小3倍
//		result= UtilsClass.comp(result);
            Canvas canvas = new Canvas(result);
            Rect rect = new Rect(0, 0, bit.getWidth(), bit.getHeight());// original
            rect = new Rect(0, 0, bit.getWidth() / 5, bit.getHeight() / 5);// 缩小3倍
            canvas.drawBitmap(bit, null, rect, null);
