...
public class BottomSelectorPopDialog extends Dialog {
    ...
    private final CharSequence[] mBtnTexts;
    ...
    private void createView() {
        LayoutInflater lLayoutInflater = mContext.getLayoutInflater();
        View view = lLayoutInflater.inflate(R.layout.imagemodule_publish_type_seletor, null);
        setContentView(view, new ViewGroup.LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT, ViewGroup.LayoutParams
                .WRAP_CONTENT));
        ...
        for (int i = 0; i < mBtnTexts.length; i++) {
            Button lBtn = new Button(mContext);

            lBtn.setTextSize(TypedValue.COMPLEX_UNIT_SP, 22);
            lBtn.setText(mBtnTexts[i]);
